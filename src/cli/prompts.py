...
class InteractivePrompts:
    ...
    def prompt_technology_categories(self) -> List[TechnologyCategory]:
        ...
        for category in TechnologyCategory:
            ...
            choice_name = f"{category.value.replace('_', ' ').title()} ({count} components)"
            category_choices.append(questionary.Choice(title=choice_name, value=category))
    ...
    def prompt_resolve_dependencies(self, missing_deps: List[TechnologyComponent]) -> List[str]:
        """
    ...
    def prompt_integration_flows(self, suggested_flows: List) -> List[str]:
        ...
        selected = questionary.checkbox(
            "Select integration flows to include:",
            choices=choices
        ).ask()
    ...
    def prompt_diagram_configuration(self) -> Dict[str, Any]:
        ...
        config['filename'] = questionary.text(
            ...
            default="microsoft_architecture"
        ).ask()
        ...
        show_advanced = questionary.confirm(
            "Configure advanced diagram options?",
            default=False
        ).ask()

        if show_advanced:
            config['show_descriptions'] = questionary.confirm(
                "Show component descriptions in diagram?",
                default=False
            ).ask()

            config['color_by_category'] = questionary.confirm(
                "Color components by category?",
                default=True
            ).ask()
    ...
    def display_selection_summary(self, selection_service: SelectionService) -> None:
        ...
        table = Table(title="Architecture Summary", show_header=True, header_style="bold blue")
    ...
    def prompt_components_to_remove(self, current_components: List[TechnologyComponent]) -> List[str]:
        """
