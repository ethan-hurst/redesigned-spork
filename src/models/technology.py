...
class TechnologyComponent(BaseModel):
    ...
    layer: LayerType = Field(..., description="Architectural layer this component belongs to")
    icon_path: Optional[str] = Field(None, description="Path to the component icon")
    dependencies: List[str] = Field(default_factory=list, description="List of component IDs this depends on")
    conflicts: List[str] = Field(default_factory=list, description="List of component IDs that conflict with this")
    integration_patterns: List[IntegrationPattern] = Field(
        default_factory=list,
        description="List of supported integration patterns"
    )
    is_core: bool = Field(default=False, description="Whether this is a core/foundational component")
    pricing_tier: Optional[str] = Field(None, description="Pricing tier or licensing requirement")
...
class IntegrationFlow(BaseModel):
    ...
    integration_pattern: IntegrationPattern = Field(..., description="Integration pattern used")
    description: str = Field(..., description="Description of what flows between components")
    bidirectional: bool = Field(default=False, description="Whether the flow is bidirectional")
...
class TechnologyStack(BaseModel):
    ...
    integration_flows: List[IntegrationFlow] = Field(
        default_factory=list,
        description="List of integration flows between components"
    )
    ...
    def add_component(self, component: TechnologyComponent) -> bool:
        ...
        for existing in self.components:
            if component.conflicts_with(existing.id) or existing.conflicts_with(component.id):
                raise ValueError(f"Component {component.id} conflicts with {existing.id}")
    ...
    def remove_component(self, component_id: str) -> bool:
        ...
        self.integration_flows = [
            flow for flow in self.integration_flows
            if flow.source_component_id != component_id and flow.target_component_id != component_id
        ]
        ...
        for component in self.components:
            for dependency in component.dependencies:
                ...
                if dep_component:
                    ...
                    common_patterns = set(component.integration_patterns) & set(dep_component.integration_patterns)
                    if common_patterns:
                        ...
                        if not any(f.id == flow_id for f in self.integration_flows):
                            suggestions.append(IntegrationFlow(
                                id=flow_id,
