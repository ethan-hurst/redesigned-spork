...
from models.technology import LayerType, TechnologyComponent, TechnologyStack
...
class Architecture(BaseModel):
    ...
    technology_stack: TechnologyStack = Field(
        ..., description="The selected technology components"
    )
    layout: DiagramLayout = Field(
        default=DiagramLayout.LAYERED, description="Diagram layout style"
    )
    ...
    show_integration_flows: bool = Field(
        default=True, description="Whether to show integration flows"
    )
    show_layer_labels: bool = Field(
        default=True, description="Whether to show layer labels"
    )
    show_component_descriptions: bool = Field(
        default=False, description="Whether to show component descriptions"
    )
    ...
    def validate_architecture(self) -> list[str]:
        ...
        for flow in self.technology_stack.integration_flows:
            ...
            if not source:
                errors.append(
                    f"Integration flow {flow.id} references missing source component {flow.source_component_id}"
                )
            if not target:
                errors.append(
                    f"Integration flow {flow.id} references missing target component {flow.target_component_id}"
                )

            # Validate integration pattern compatibility
            if source and not source.supports_integration(flow.integration_pattern):
                errors.append(
                    f"Component {source.id} does not support integration pattern {flow.integration_pattern}"
                )
            if target and not target.supports_integration(flow.integration_pattern):
                errors.append(
                    f"Component {target.id} does not support integration pattern {flow.integration_pattern}"
                )

        # Check for minimum components in critical layers
        if (
            LayerType.DATA not in self.layer_organization
            or not self.layer_organization[LayerType.DATA]
        ):
            errors.append(
                "Architecture should include at least one data layer component"
            )
    ...
    def get_layer_order(self) -> list[LayerType]:
        ...
        return [
            layer
            for layer in canonical_order
            if layer in self.layer_organization and self.layer_organization[layer]
        ]
    ...
    def get_integration_complexity_score(self) -> int:
        ...
        score = min(
            10, max(1, (component_count // 3) + (flow_count // 2) + layer_count)
        )
...
class DiagramConfig(BaseModel):
    ...
    # Output settings
    format: DiagramFormat = Field(
        default=DiagramFormat.PNG, description="Output format"
    )
    ...
    # Visual settings
    width: int = Field(
        default=1200, description="Diagram width in pixels", ge=400, le=4000
    )
    height: int = Field(
        default=800, description="Diagram height in pixels", ge=300, le=3000
    )
    dpi: int = Field(default=300, description="DPI for raster formats", ge=72, le=600)

    # Layout settings
    node_spacing: float = Field(
        default=1.0, description="Spacing between nodes", ge=0.5, le=3.0
    )
    layer_spacing: float = Field(
        default=2.0, description="Spacing between layers", ge=1.0, le=5.0
    )

    # Style settings
    show_icons: bool = Field(
        default=True, description="Whether to show component icons"
    )
    show_labels: bool = Field(
        default=True, description="Whether to show component labels"
    )
    show_descriptions: bool = Field(
        default=False, description="Whether to show component descriptions"
    )
    color_by_category: bool = Field(
        default=True, description="Whether to color components by category"
    )

    # Microsoft branding
    use_microsoft_colors: bool = Field(
        default=True, description="Whether to use Microsoft brand colors"
    )
    show_microsoft_logo: bool = Field(
        default=False, description="Whether to include Microsoft logo"
    )
...
class DiagramMetadata(BaseModel):
    ...
    layers_included: list[LayerType] = Field(
        ..., description="Layers included in the diagram"
    )
    ...
    generation_time_seconds: Optional[float] = Field(
        None, description="Time taken to generate diagram"
    )
    ...
    def summary(self) -> str:
        ...
        """
        layer_names = [
            layer.value.replace("_", " ").title() for layer in self.layers_included
        ]
        return (
