...
def validate_component_id(component_id: str) -> Tuple[bool, Optional[str]]:
    ...
    if not re.match(r'^[a-z][a-z0-9_]*$', component_id):
        return False, "Component ID must start with a letter and contain only lowercase letters, numbers, and underscores"
...
def validate_component_list(component_ids: Union[str, List[str]]) -> Tuple[bool, Optional[str], List[str]]:
    """
...
def validate_output_format(format_str: str) -> Tuple[bool, Optional[str]]:
    ...
    if format_str.lower() not in valid_formats:
        return False, f"Invalid format '{format_str}'. Valid formats: {', '.join(valid_formats)}"

    return True, None


def validate_spacing_value(value: float, min_value: float = 0.1, max_value: float = 10.0) -> Tuple[bool, Optional[str]]:
    """
