...
class ArchitectureGenerator:
    ...
    def generate_architecture(
        ...
        try:
            ...
            if not is_valid:
                logger.warning(
                    f"Generating architecture with validation errors: {errors}"
                )
            ...
            logger.info(
                f"Generated architecture '{architecture.name}' with {len(technology_stack.components)} components"
            )
            return architecture
    ...
    def enhance_architecture_with_best_practices(
        ...
        if not has_security:
            suggestions.append(
                "Consider adding Azure Active Directory for identity management"
            )

        # Check for monitoring components
        has_monitoring = any(
            "monitor" in c.id or "insights" in c.id for c in components
        )
        if not has_monitoring:
            suggestions.append(
                "Consider adding Azure Application Insights for monitoring"
            )
        ...
        if not has_data_layer:
            suggestions.append(
                "Consider adding a data storage component like Dataverse"
            )
        ...
        if has_power_apps and not has_power_automate:
            suggestions.append(
                "Consider Power Automate for workflow automation with Power Apps"
            )

        # Dynamics 365 specific recommendations
        dynamics_components = [
            c for c in components if c.category.value == "dynamics_365"
        ]
        if dynamics_components and not has_dataverse:
            suggestions.append(
                "Dynamics 365 applications require Dataverse for data storage"
            )
        ...
        if len(components) > 3 and not has_integration:
            suggestions.append(
                "Consider adding integration services like Azure Logic Apps or Power Automate"
            )
    ...
    def generate_layer_recommendations(
        self, architecture: Architecture
    ) -> dict[LayerType, list[str]]:
        """
        ...
        for layer, components in layer_matrix.items():
            ...
            if layer == LayerType.PRESENTATION:
                if not components:
                    layer_recommendations.append(
                        "Add user interface components like Power Apps or Power Pages"
                    )
                elif len(components) == 1:
                    layer_recommendations.append(
                        "Consider multiple channels for better user experience"
                    )

            elif layer == LayerType.APPLICATION:
                if not components:
                    layer_recommendations.append(
                        "Add business logic components like Dynamics 365 applications"
                    )
                else:
                    # Check for AI/Copilot components
                    has_ai = any(
                        "copilot" in c.name.lower() or "ai" in c.name.lower()
                        for c in components
                    )
                    if not has_ai:
                        layer_recommendations.append(
                            "Consider adding AI Builder or Copilot Studio for intelligent features"
                        )

            elif layer == LayerType.INTEGRATION:
                if len(architecture.technology_stack.components) > 2 and not components:
                    layer_recommendations.append(
                        "Add integration services like Power Automate or Azure Logic Apps"
                    )
                elif components:
                    ...
                    if len(components) > 2 and not has_api_mgmt:
                        layer_recommendations.append(
                            "Consider Azure API Management for enterprise scenarios"
                        )

            elif layer == LayerType.DATA:
                if not components:
                    layer_recommendations.append(
                        "Add data storage components like Dataverse or Azure Data Lake"
                    )
                else:
                    # Check for analytics
                    has_analytics = any(
                        "synapse" in c.id or "power_bi" in c.id for c in components
                    )
                    if not has_analytics and len(components) > 1:
                        layer_recommendations.append(
                            "Consider Power BI or Azure Synapse for analytics"
                        )

            elif layer == LayerType.SECURITY:
                if not components:
                    layer_recommendations.append(
                        "Add security components like Azure AD and Key Vault"
                    )
                else:
                    ...
                    if not has_monitoring:
                        layer_recommendations.append(
                            "Consider Azure Monitor for operational insights"
                        )
    ...
    def generate_architecture_report(self, architecture: Architecture) -> dict:
        ...
        report = {
            ...
            "summary": {
                ...
                "integration_count": len(
                    architecture.technology_stack.integration_flows
                ),
        ...
        report["recommendations"]["enhancements"] = (
            self.enhance_architecture_with_best_practices(architecture)
        )
        report["recommendations"]["by_layer"] = {
            layer.value: recs
            for layer, recs in self.generate_layer_recommendations(architecture).items()
        }
